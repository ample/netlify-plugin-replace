...
class Replace {
  ...
  async getFiles() {
    if (!this.files) {
      ...
      let ignoredFiles = this.getIgnoredFiles()
      let allowedFiles = Ignore().add(ignoredFiles).filter(Object.keys(matches))
  ...
  getIgnoredFiles(path = ".gitignore") {
    let contents = fs.existsSync(path) ? fs.readFileSync(path).toString() : ""
    if (!contents.includes("node_modules")) {
      contents += "\n\nnode_modules"
    }
    return Utils.stripCommentsAndWhitespace(contents)
      .split("\n")
      .filter(l => l !== this.dir)
      .join("\n")
  }
